...
export function ChatDetail({ chatId, chatName, chatAvatar, onBack }: ChatDetailProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#efeae2] dark:bg-[#0b141a]">
      ...
      {/* Input Area Skeleton */}
      <div className="p-3 bg-light-secondary dark:bg-[#202c33] flex items-end gap-2">
        {showEmojiPicker && (
        ...
        {inputText.trim() || pastedImage || selectedFile ? (
          ...
          >
            <svg
              ...
              width="24"
              height="24"
              className="fill-current text-light-primary dark:text-light-primary"
            >
...
function MediaContent({
  ...
  const getThumbnail = () => {
    ...
    if (type === "image") thumb = message.Content?.imageMessage?.JPEGThumbnail
      ...
      thumb =
        (message.Content?.stickerMessage as any)?.jpegThumbnail ||
        (message.Content?.stickerMessage as any)?.pngThumbnail ||
        (message.Content?.stickerMessage as any)?.PNGImage
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (type === "image") mime = message.Content?.imageMessage?.mimetype || "image/jpeg"
        ...
        mime = message.Content?.documentMessage?.mimetype || "application/pdf"
    ...
    if (type === "image") {
      ...
      return (
        <img
          ...
          style={{ width: 195, height: 195, borderRadius: 12 }}
        />
      )
    } else if (type === "video") {
      ...
      {thumbnail &&
        (type === "sticker" ? (
          <img
            ...
            className={`object-contain ${loading ? "opacity-50" : ""}`}
            style={{ width: 195, height: 195, borderRadius: 12 }}
          />
        ) : (
          <img
            src={thumbnail}
            alt="Thumbnail"
            className={`max-w-full rounded-lg ${loading ? "opacity-50" : ""}`}
          />
        ))}
      {!thumbnail && (
        <div
          className={
            type === "sticker"
              ...
              : "w-64 h-64 bg-gray-200 dark:bg-gray-700 flex items-center justify-center rounded-lg"
          }
          style={type === "sticker" ? { width: 195, height: 195, borderRadius: 12 } : undefined}
        >
...
function QuotedMessage({ contextInfo }: { contextInfo: any }) {
  ...
  if (quoted.conversation) text = parseWhatsAppMarkdown(quoted.conversation)
    ...
    text = parseWhatsAppMarkdown(quoted.extendedTextMessage.text)
  ...
  return (
    <div className="bg-black/5 dark:bg-white/10 rounded-md p-2 mb-2 border-l-4 border-light-primary dark:border-light-primary text-sm cursor-pointer">
      <div className="font-bold text-light-primary dark:text-light-primary text-xs mb-1">
        {name}
      </div>
...
function MessageItem({
  ...
  const timeStr = new Date(message.Info.Timestamp).toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  })

  let senderName = message?.Info?.PushName ?? message?.Info?.Sender?.toString?.() ?? "Unknown"
  ...
  if (sender && typeof sender.toString === "function" && !sender.toString().endsWith("lid")) {
    GetContact(sender)
      .then((contact: any) => {
        senderName = contact?.full_name ?? senderName ?? sender.toString()
      })
      .catch(() => {
  ...
  const contextInfo =
    message.Content?.extendedTextMessage?.contextInfo ||
  ...
  if (message.Content?.conversation) {
    ...
    content = (
      <div>
        <MediaContent
          ...
          sentMediaCache={sentMediaCache}
        />
        {message.Content.imageMessage.caption && (
          <div className="mt-1">{parseWhatsAppMarkdown(message.Content.imageMessage.caption)}</div>
        )}
      </div>
    )
    ...
    content = (
      <div>
        <MediaContent
          ...
          sentMediaCache={sentMediaCache}
        />
        {message.Content.videoMessage.caption && (
          <div className="mt-1">{parseWhatsAppMarkdown(message.Content.videoMessage.caption)}</div>
        )}
      </div>
    )
    ...
    message.Content?.pollCreationMessage ||
    message.Content?.pollCreationMessageV2 ||
    message.Content?.pollCreationMessageV3
  ) {
    content = "📊 Poll"
  } else {
          ...
          <div className={`w-full flex ${isMe ? "justify-end" : "justify-start"} mt-2`}>
            <div className="bg-white dark:bg-[#0b141a] rounded-full px-3 py-1 flex items-center gap-2 text-[11px] text-gray-600 dark:text-gray-300">
              ...
              {isMe && (
                <span
                  className={`${
                    isTemp ? "text-gray-500" : "bg-blue-500 text-white"
  ...
  return (
    <div className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
      <div
        className={`${
          isImageOrVideo ? "max-w-[65%]" : "max-w-[70%]"
        } rounded-lg p-2 px-3 shadow-sm relative group ${
          isMe
